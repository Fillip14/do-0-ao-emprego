...
Etapa 2 · Back-end: da API ao deploy  ██░░░░░░░░░░░░░░░░░░░░   10%  🔨 S1 de 10 concluída
...
> 🔁 **Por que reiniciei:** a primeira versão da etapa fechou 4 semanas de conteúdo em 5 dias — rápido demais pra fixar. Velocidade não é aprendizado. Recomecei do zero com um plano de 10 semanas, exercício por tópico, uma pasta por semana; o trabalho da v1 está preservado em [`etapas/etapa-2/arquivo-v1/`](etapas/etapa-2/arquivo-v1/). Plano completo em [`etapas/etapa-2/plano.md`](etapas/etapa-2/plano.md).

| Semana | Assunto | Status |
|:---:|---|:---:|
| **S1** | Node — HTTP, `node:http`, event loop, npm, ESM | ✅ |
| **S2** | Express — rotas, middleware, validação, erro central, supertest | ⏳ |
| **S3** | TypeScript — strict, narrowing, generics, utility types | ⬜ |
| **S4** | PostgreSQL — SQL, `pg`, injection, transações | ⬜ |
| **S5** | Testes a fundo — pirâmide, mocks, cobertura, TDD | ⬜ |
| **S6** | Camadas + paginação, filtros, zod | ⬜ |
| **S7** | Migrations + ORM | ⬜ |
| **S8** | Auth + segurança de borda | ⬜ |
| **S9** | Deploy — banco gerenciado, health, logs, auto-ataque | ⬜ |
| **S10** | Docker + CI | ⬜ |
...
| 20–21/07 | **S1 — Node (12 exercícios)** — servidor como processo escutando uma porta · anatomia do HTTP no `curl -i` · `node:http` cru com roteamento e status corretos · streams: corpo chegando em 32 chunks de 64 KB · event loop bloqueado por `while` síncrono, medido com `curl -w` · `uncaughtException` × `unhandledRejection` · porta por variável de ambiente · package.json linha a linha, semver e lock · ESM × CommonJS · `crypto`/`path`/`fs promises` · breakpoint com `--inspect` no VS Code · Vitest com funções puras |
...
│   └── etapa-2/             → plano (10 semanas), devlog, arquivo-v1/ (trabalho de 16–20/07)
│                              e uma pasta por semana (semana-01-node/…semana-10-docker-ci/)
