...
> 🔁 **Por que reiniciei:** a primeira versão da etapa fechou 4 semanas de conteúdo em 5 dias — rápido demais pra fixar. Velocidade não é aprendizado. Recomecei do zero com um plano de 10 temas, exercício por tópico, uma pasta por tema; o trabalho da v1 está preservado em [`etapas/etapa-2/arquivo-v1/`](etapas/etapa-2/arquivo-v1/). Plano completo em [`etapas/etapa-2/plano.md`](etapas/etapa-2/plano.md).
...
| Data | Conteúdo estudado |
|:---:|---|
| 16–20/07 | **v1 da etapa (arquivada)** — sob o plano antigo de 5 semanas com entregável semanal, 4 "semanas" fecharam em 5 dias: API Express em memória testada no Bruno (S1) · CRUD completo com validação, erro centralizado e 15 testes Vitest/supertest (S2) · PostgreSQL com queries parametrizadas, SQL injection demonstrada de verdade e banco de teste isolado (S3) · migração TypeScript `strict` com 16 testes verdes (S4). O código funcionou, mas velocidade não virou retenção — **etapa reiniciada em 21/07** com plano novo de 10 semanas; código e detalhes da v1 em [`etapas/etapa-2/arquivo-v1/`](etapas/etapa-2/arquivo-v1/) |
| 20/07 | **T1 — Node (Ex 01–02)** — o servidor como processo escutando uma porta · anatomia do HTTP lida no `curl -i` |
| 21/07 | **T1 — Node (Ex 03–12)** — `node:http` cru · streams em chunks · event loop travado por `while` síncrono · `uncaughtException` × `unhandledRejection` · porta por variável de ambiente · package.json e semver · ESM × CommonJS · `path`/`fs`/`crypto` · `--inspect` · Vitest.<br>**T2 — Express (Ex 01–11)** — o que o framework tira do caminho · `express.json()` e as armadilhas do `req.body` · `params` × `query` × `body` · validação na borda e definição do **formato de erro da API** · `Router` com prefixo · tabela do CRUD e os porquês de 201/204/400/404 · idempotência · `Location` e 405 com `Allow` · middleware com `res.on('finish')` e ordem de registro · `morgan` × logger próprio · **tratador de erro central** e 404 coringa |
| 22/07 | **T2 — Express (Ex 12–13)** — erro em handler `async` e a diferença entre Express 4 e 5 · `app` separado do `server` + suíte com **supertest**. Fechou com um bug achado por raciocínio, não por teste: `POST` sem `Content-Type` derrubava o handler em 500 — corrigido e travado com teste de regressão.<br>**T3 — TypeScript aberto** — enunciados dos 14 exercícios do tema. Revisão crítica do enunciado antes de começar derrubou a primeira versão: exercícios que se contradiziam, config apontando pra pasta inexistente, exemplo de `satisfies` incorreto e — o principal — 13 exercícios de playground sem nada que rodasse no fim. **Plano da etapa ajustado:** a API agora é progressiva tema a tema, e o T3 fecha portando a API do T2 para TypeScript |
...
│   └── etapa-2/             → plano (10 temas), devlog, arquivo-v1/ (trabalho de 16–20/07)
│                              e uma pasta por tema (t01-node/…t10-docker-ci/)
