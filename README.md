...
<p align="center">
  ...
  <img alt="Etapa atual" src="https://img.shields.io/badge/etapa%20atual-1%20de%205-3178c6?style=for-the-badge">
...
<p align="center">
  ...
  <a href="#-progresso-da-etapa-1">Progresso</a> ·
...
> Planejamento completo, cronograma e critérios de cada etapa estão em [`docs/planos/`](docs/planos/).
...
Etapa 1 · JS sólido + fundamentos web █████████████████░░░░░   78%  🔨 em andamento
Etapa 2 · Back-end (Node/Express/TS)  ░░░░░░░░░░░░░░░░░░░░░░    0%  🔒 a seguir
...
| **1** | JS sólido + fundamentos web | Gerenciador de tarefas **web** (HTML/CSS/DOM) | 🔨 Em andamento |
| **2** | Back-end | API REST de tarefas + PostgreSQL + deploy | 🔒 A seguir |
...
**JavaScript sólido + fundamentos web** · avaliação **CONFIRMADA para 21/07** (escrita + oral, sem consulta).

```
Temas selados:  14 / 18   █████████████████░░░░░   78%
```

Cada tema só é "selado" quando passa pelas 5 travas do **DoD** (Definition of Done): testes verdes na hora com caso negativo · revisão zerada · devlog no ato · pergunta-selo respondida sem consulta · commit com push.

<details>
<summary><strong>🧩 Bloco de linguagem (T1–T12)</strong></summary>

| Tema | Conteúdo | Status |
|:---:|---|:---:|
| T1 | Tipos e coerção (`typeof`, `==` vs `===`, truthy/falsy) | ✅ |
| T2 | Funções a fundo (arrow, closure, escopo, rest) | ✅ |
| T3 | Strings e regex | ✅ |
| T4 | Arrays I: mutação e referência | ✅ |
| T5 | Arrays II: `map`, `filter`, `find` | ✅ |
| T6 | `reduce` e `sort` | ✅ |
| T7 | Objetos (`keys/values/entries`, desestruturação, spread) | ✅ |
| T8 | JSON e arquivos (Node, `fs`, CommonJS vs ESM) | ✅ |
| T9 | Erros e validação (`throw`, `try/catch`, `assert.throws`) | ✅ |
| T10 | Assíncrono I: callbacks, `setTimeout`, Promise | ✅ |
| T11 | Assíncrono II: `async/await` e `fetch` | ✅ |
| T12 | OO: `this`, classes, protótipos | ✅ |

</details>

<details open>
<summary><strong>🌐 Bloco web + entrega (T13–T18)</strong></summary>

| Tema | Conteúdo | Status |
|:---:|---|:---:|
| T13 | HTML/CSS funcional (página estática do task manager) | ✅ |
| T14 | DOM e eventos (dar vida à página) | ✅ |
| T15 | HTTP na prática (página busca-CEP) | ⏳ Próximo |
| T16 | Revisão espaçada + planejamento do projeto | ⬜ |
| T17 | Projeto: gerenciador de tarefas **web** | ⬜ |
| T18 | Acabamento e entrega | ⬜ |

</details>

**Legenda:** ✅ selado · ⏳ em andamento · ⬜ a fazer · 🔒 bloqueado
...
| 14/07 | **T12–T14** — OO (`this`, classes, protótipos) + HTML/CSS (tags semânticas, box model, flexbox) + DOM e eventos (`querySelector`, `createElement`, `addEventListener`, `preventDefault`) | [Assistir](https://www.twitch.tv/videos/2819777408) | — |
...
├── devlog/                  → diário técnico de cada etapa (matéria-prima do catálogo)
├── docs/
│   ├── planos/              → planejamento, cronograma geral e cronograma de etapas
│   ├── avaliacoes/          → avaliações das etapas
│   └── resumo-metodos-etapa-1.md
├── avaliacao-de-nivel/      → avaliação de nível inicial e correção
└── etapas/                  → código dos exercícios e projetos, uma pasta por tema (t01/, t02/…)
```
...
| 2 | **Revisor** — code review após cada feature | Ler código criticamente |
...
<p align="center">
  <strong>Estado atual:</strong> Etapa 1 em andamento — 14/18 temas selados · avaliação em 21/07.
</p>
