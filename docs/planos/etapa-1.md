...
- **Ex 1.1** — `tipoDe(valor)`: retorna `"numero"`, `"texto"`, `"booleano"`, `"nulo"`, `"indefinido"`, `"lista"` ou `"objeto"`. Atenção: `typeof null` e arrays têm pegadinhas.
- **Ex 1.2** — Arquivo `previsoes.js` com 15 expressões (ex.: `"1" + 1`, `"1" - 1`, `null == undefined`, `[] + []`, `0 == false`...). Escreva a previsão em comentário ANTES de rodar. Depois rode e anote os erros de previsão no devlog.
...
- **Ex 1.3** — `criarContador()`: retorna uma função que a cada chamada retorna 1, 2, 3... Dois contadores criados separadamente não se misturam (teste isso).
- **Ex 1.4** — `aplicar(fn, lista)`: retorna nova lista com `fn` aplicada a cada item, SEM usar `.map` (loop manual). Teste passando duas funções diferentes.
...
- **Ex 1.5** — `formatarNome(nome)`: `"  joão da SILVA "` → `"João Da Silva"`.
- **Ex 1.6** — Refazer `validarSenha` (Ex 0.5) usando regex nas verificações. Mesmo contrato: `{ valida, erros }`.
...
- **Ex 1.7** — `previsoes-arrays.js`: 10 trechos que mutam ou copiam arrays. Prever a saída em comentário antes de rodar. Depois: `copiarSemUltimo(lista)` que retorna cópia sem o último item, SEM alterar a original (teste que a original não mudou).
...
- **Ex 1.9** — `estatisticas(notas)`: retorna `{ media, maior, menor, aprovados }` (aprovado ≥ 7) usando reduce/filter. Lista vazia retorna `null`. Ordene as notas do maior para o menor.

## Dia 7 — Qua 15/07 — Objetos

Estudar: acesso com `.` e `[]`, `Object.keys/values/entries`, desestruturação, spread em objetos.

- **Ex 1.10** — `contarPalavras(frase)`: refazer o ex04 da avaliação em JS. Retorna array de pares `[palavra, quantidade]` ordenado da mais frequente para a menos.
...
- **Ex 1.11** — Criar `produtos.json` com os produtos do Ex 1.8. Programa que lê o arquivo, filtra os com estoque e salva em `com-estoque.json`. Funções em um módulo separado, importadas no arquivo principal.

## Dia 9 — Sex 17/07 — Erros e validação

Estudar: `throw`, `try/catch`, `assert.throws`, mensagens de erro úteis.

- **Ex 1.12** — `parseIdade(texto)`: converte texto em número inteiro de idade. Lança erro com mensagem clara se: não for número, for negativo, for decimal. Testes cobrindo os 3 erros com `assert.throws` + casos válidos.
...
- **Ex 1.13** — `esperar(ms)`: retorna uma Promise que resolve depois de `ms` milissegundos. Usar para imprimir "1"... "2"... "3" com 1s de intervalo, primeiro com `then`, depois encadeado.

## Dia 11 — Dom 19/07 (4h) — Assíncrono parte 2: async/await e fetch

Estudar: `async/await`, `try/catch` com await, `fetch` no Node.

- **Ex 1.14** — `buscarCep(cep)`: consulta `https://viacep.com.br/ws/{cep}/json/` e retorna `{ rua, bairro, cidade, uf }`. CEP inexistente ou mal formado lança erro com mensagem clara. Testar com CEP válido e inválido.
...
```
node tarefas.js adicionar "estudar reduce"
node tarefas.js listar
node tarefas.js concluir 2
node tarefas.js apagar 3
```

- Persistência em `tarefas.json`. Lógica em módulo separado (`lib.js`) com funções puras; `tarefas.js` só lê argv e chama.
