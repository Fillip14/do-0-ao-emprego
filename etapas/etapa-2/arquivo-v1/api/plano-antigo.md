...
> Refeito em 16/07/2026 do zero, usando apenas o cronograma-mestre (`docs/cronograma-etapas.md`) e as regras vivas do projeto. Organização própria desta etapa: **semanas com um entregável que funciona cada** — no back-end, "estudei" não conta; "está no ar e responde" conta.
>
> **Alterado em 20/07:** eram 5 semanas; viraram 6. As S1–S4 fecharam em 5 dias corridos, e a folga foi convertida numa **semana de consolidação** (nova S5) antes do deploy, que passou a ser a S6.
...
## Semana 5 (21–27/07) — Consolidação: reconstruir do zero

> **Inserida em 20/07**, a pedido do Fillip, depois do checkpoint amarelo da S4: *"consegui fazer, mas ainda preciso estudar muito mais todos os assuntos das semanas"*. S1–S4 fecharam em 5 dias corridos; a folga de cronograma (4 semanas para 1 entregável) é gasta aqui, em profundidade. O deploy virou a Semana 6.
>
> **Método: reconstrução, não releitura.** Reler o próprio código dá a sensação de saber, porque a resposta está na tela. O que revela buraco de verdade é escrever de novo com a página em branco. Nenhum conteúdo novo esta semana.

**Regra da semana:** pasta nova `api-recall/`, do zero. **Proibido abrir o código da `api/`.** Documentação oficial (Express, node-postgres, Vitest, TypeScript) é permitida — ela existe no trabalho real. Travou mais de 15 minutos? Consulte a `api/`, resolva, e **anote o que precisou olhar** — essa anotação é o entregável mais valioso da semana.

**Bloco A — Esqueleto e rotas.** Projeto npm do zero, TypeScript e `strict` desde o primeiro arquivo (não migrar depois — nascer tipado), Express, `app`/`server` separados, as 5 rotas de `/tasks` respondendo com dados fixos. Sem banco ainda.

**Bloco B — Validação e erros.** Middlewares de validação, formato de erro consistente, 404 coringa e handler de erro central — tudo de memória, incluindo o contrato (status e corpo de cada caso). Ao final, compare o contrato que você escreveu com o do README da `api/`: divergência = conceito que não estava firme.

**Bloco C — Banco.** Schema escrito na mão no `psql` (tipos e restrições justificados de novo, sem copiar o `schema.sql`), `db.ts` com pool único, as 5 rotas persistindo, todas as queries parametrizadas.

**Bloco D — Testes e prestação de contas.** Suíte cobrindo sucesso e erro de todas as rotas, banco de teste isolado, `tsc --noEmit` limpo. Depois: `diff` entre a `api-recall/` e a `api/` — e a lista dos buracos, que vira o roteiro de revisão para a avaliação.

**Entregável 5 — a mesma API, construída de novo sem consultar:**

- `api-recall/` funcionando: 5 rotas, TypeScript `strict`, PostgreSQL, validação, formato de erro consistente, suíte verde.
- `consolidacao.md` com a lista honesta do que você precisou consultar e do que divergiu da versão original — por assunto (Express, SQL, TS, testes).
- Um parágrafo por assunto explicando o conceito com suas palavras, sem olhar o código. Se não sair o parágrafo, o assunto não está aprendido.

> Ao fim da semana, decidimos juntos: os buracos apontam para revisão focada antes da avaliação, ou já estão fechados?

## Semana 6 (a partir de 28/07) — Deploy, robustez e entrega
