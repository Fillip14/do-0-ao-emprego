...
9. Generics + `strictNullChecks` na prática. — 🔨 **Ex:** `firstItem<T>` e um embrulho `ApiResponse<T>` com o tipo fluindo sem `as`; e um `find()` cujo `undefined` o compilador obriga a tratar (ensaio do `rows[0]` do Tema 4).
