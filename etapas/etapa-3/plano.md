...
4. **Um tema só fecha quando os quatro estão feitos:** (a) o que o tema entrega está na `web/`, rodando no navegador; (b) `npm run typecheck` limpo e — a partir do T14 — `npm test` verde; (c) revisão da Parte B feita e as correções aplicadas; (d) questionário da Parte C respondido no devlog, tudo commitado e no push.
