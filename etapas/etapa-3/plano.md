...
1. **Trilha de IA — fase PAR DE PROGRAMAÇÃO.** Liberada a geração de **trechos pequenos** (uma função, um componente, um tipo) com **duas condições inegociáveis**: (a) você entende cada linha antes de commitar — se não entende, pergunta ou reescreve; (b) existe prova de que funciona. Até o Tema 14 a prova pode ser verificação manual registrada no devlog (o que você abriu, clicou e viu); do Tema 14 em diante é **teste automatizado**. Continua **proibido**: pedir "monta a tela pra mim", pedir o arquivo inteiro, colar sem ler.
...
4. **Um tema só fecha quando os quatro estão feitos:** (a) o que o tema entrega está na `web/`, rodando no navegador; (b) `npm run typecheck` limpo e — a partir do T14 — `npm test` verde; (c) revisão da Parte B feita e as correções aplicadas; (d) defesa oral da Parte C feita e o que ficou de pé registrado no devlog, tudo commitado e no push.
...
6. **`studie-tNN-tema.md` na abertura de cada tema**, em três partes. **Parte A:** a IA detalha cada tópico de estudo na estrutura *1- O que resolve? 2- Quando usar? 3- Exemplo pequeno. 4- Armadilhas.* **Parte B:** alterações no app — *1- Preparação do ambiente* (setup de ferramenta é a única coisa que a IA entrega pronta; é atrito, não aprendizado); *2- O que do tema deve aparecer na `web/`*; *3- Critérios*; *4- Aguardar execução*; *5- Revisão do código* (apontar de forma simples onde estão os erros e o que faltou, para eu corrigir). **Parte C — defesa oral 6 a 8 perguntas**, eu respondo falado e curto (2–3 frases); a IA contra-argumenta em cima da minha resposta, e o que não se sustentar me manda de volta à Parte A naquele tópico. No devlog vai **uma linha por pergunta, só o que ficou de pé** — o que caiu entra corrigido e marcado com ⚠️, e é essa marca que vira revisão espaçada. O que é consulta (nome de atributo, diferença de pasta, história de ferramenta) **não vira pergunta**: fica na Parte A, para quando eu esbarrar nela trabalhando.
7. **Autonomia na aplicação.** A Parte A é consulta, a Parte B é enunciado — não roteiro. Aberto o tema, eu aplico sozinho na `web/`: decido o desenho, erro, corrijo e commito sem pedir passo a passo. A IA só volta a falar na revisão (Parte B item 5) e no questionário (Parte C). Se eu travar, a pergunta é minha — e agora, na fase Par, ela pode vir com um trecho pequeno junto, sujeito à regra 1.
