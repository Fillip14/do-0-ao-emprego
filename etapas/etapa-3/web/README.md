...
> **Exceção registrada em 11/08:** este README só devia mudar na emenda dos Temas 8/9 da Etapa 2 — mas a Etapa 2 encerrou incompleta nessa data sem fazer essa emenda, e a limitação abaixo passou a existir por causa disso. Ficou registrada aqui pra não enganar quem ler. Detalhe em [`../../etapa-2/plano.md`](../../etapa-2/plano.md#encerramento).

- **Desde 11/08 o CRUD contra a API local está quebrado.** `/tasks` passou a exigir `Authorization: Bearer <token>` (Tema 8 da Etapa 2); este front nunca chegou a mandar token — `RequireAuth` continua fixo em `isAuthenticated = true`, sem tela de login. Toda chamada a `/tasks` hoje recebe `401`.
...
- **O erro de validação do servidor já cai no campo do lado da API, mas isso nunca foi verificado por aqui.** Tema 6 da Etapa 2 (11/08) trocou o erro único (`field: 'task'`) por um erro por campo — confirmado via curl, não pela tela. O `ApiError.fieldErrors` já sabia ler esse formato desde que foi escrito, sem cliente; falta só rodar o fluxo de verdade contra o front, e isso esbarra na limitação de auth acima.
